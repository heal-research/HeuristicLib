...
Use the [Glossary](/guide/glossary) for terminology, [Core concepts](/guide/fundamentals/core-concepts) for the public model and the focused topic pages for detailed behavior. Incomplete work and unresolved decisions belong in `plans/developer-backlog.md`.
...
Detailed implementation gaps and open design decisions are tracked in `plans/developer-backlog.md`.
