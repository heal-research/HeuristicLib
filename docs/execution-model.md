...
Operator duration is a different time budget unit. Helpers such as `WithMaxEvaluatorDuration(evaluator, ...)`, `WithMaxMutatorDuration(...)`, `WithMaxRefinerDuration(...)` and `WithMaxCrossoverDuration(...)` measure only active work inside observed operator calls, using a before call, inner call and after call measurement. If a caller pauses between pulling states from a stream, whole run elapsed time continues to pass, but operator duration does not increase while the observed operator is not running. Duration is recorded even if the observed operator call throws because the failed call still consumed observed work time. Like other external operator budgets, the operator call that crosses the duration budget finishes, its produced state is yielded and only future stream consumption stops.
...
- Typed operator-budget helpers such as `WithMaxMutatorCalls(...)`, `WithMaxMutatedCandidates(...)`, `WithMaxRefinerCalls(...)`, `WithMaxRefinedCandidates(...)`, `WithMaxSelectedCandidates(...)`, `WithMaxReplacementCandidates(...)`, and `WithMaxSelectorDuration(...)` observe an explicitly supplied operator and install the matching counted or measured replacement for that run.
