...
| Part | Responsibility |
| --- | --- |
| `ExpressionTree` | The immutable candidate. See [symbolic expressions](symbolic-expressions.md). |
...
- `Symbols.FixedConstant(value)` — a constant such as `pi`. It never local-mutates,
  though ordinary terminal replacement can still replace it. A fixed constant is *not*
  an evolvable constant with a zero-width distribution.
- `Symbols.Constant(initialDistribution, perturbation)` — an evolvable constant,
...
| `Chain(perturbations)` | apply in sequence, atomically — if a stage is inapplicable, nothing changes |
...
A perturbation that samples a no-op still counts as success — no retry is performed to
force a changed offspring.
...
containment, mutation, or crossover. See [data analysis](data-analysis.md#linear-scaling).
...
Refinement is normally the most expensive thing a run does — commonly over 95 percent
of wall-clock time — so the settings that matter most are how many candidates are
...
[operator composition](operator-composition.md#refiner-composition).
...
Compile explicitly when one expression is evaluated many times — scoring a final
model, or predicting repeatedly in production:
...
Pass an explicit symbol when several would match — `Variable(name, variableSymbol)`.
...
`ToRegressor` performs the one-time compilation the returned `SymbolicRegressor`
retains. Decorators compose normally: `ToBounded(lower, upper)` constrains outputs,
and `FitLinearScaling(data)` fits and retains affine scaling for later prediction —
the step that carries search-time linear scaling into a final model.
