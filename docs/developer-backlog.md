...
- find a way to reduce the operator boilerplate that a role-contract change has to touch. The problem is unsolved, and both approaches tried so far are recorded in [Discussed, tried, and rejected](#discussed-tried-and-rejected): [typed operator invocation](#typed-operator-invocation) and [generated operator families](#generated-operator-families). Do not re-propose either without the new evidence those entries require.
  ...
  The evaluator contract change from `EvaluatedCandidate<TCandidate>` to `ObjectiveVector` (RF-2 of the symbolic-regression parameter-fitting work; see [symbolic-regression decisions](../plans/symbolic-regression-decisions.md)) is the clearest measurement available so far. It changed the single most fundamental property of an operator role — what its execution instance returns — and is worth reading from two directions at once, because the pair is the actual finding.
