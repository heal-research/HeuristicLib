...
- bring back first-class inner terminators for algorithms where users naturally expect them, especially iterative algorithms such as genetic algorithms. The current `IterativeAlgorithm` stream is effectively unbounded, and common termination such as max iterations is available through `StateTerminatedAlgorithm.WithMaxIterations(...)`, but a GA should still be able to expose a normal `Terminator`/termination configuration as part of its own API. Keep `StateTerminatedAlgorithm` only as an additional composition tool for algorithms that genuinely do not own termination, not as the only way to make ordinary iterative algorithms finite. See the plan in [../plans/termination-overhaul.md](../plans/termination-overhaul.md).
