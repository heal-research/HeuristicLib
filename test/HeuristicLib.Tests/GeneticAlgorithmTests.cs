...
public class GeneticAlgorithmTests {
  ...
  public async Task GeneticAlgorithm_ExecuteAndContinueWithOtherAlg() {
    ...
    var firstAlg = new GeneticAlgorithm<RealVector, RealVectorEncoding>(encoding, 5, creator, crossover, mutator, 0.05, evaluator, Goal.Minimize, selector, replacement, randomSource, terminationCriterion);

    var firstResult = firstAlg.Execute();

    var newTerminationCriterion = Terminator.OnGeneration(12);
    var secondAlg = new GeneticAlgorithm<RealVector, RealVectorEncoding>(encoding, 8, creator, crossover, mutator, 0.05, evaluator, Goal.Minimize, selector, replacement, randomSource, newTerminationCriterion);
