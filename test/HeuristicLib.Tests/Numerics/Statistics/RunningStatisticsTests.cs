namespace HEAL.HeuristicLib.Tests.Numerics.Statistics;
...
public sealed class RunningStatisticsTests
{
    ...
    public void Statistics_CalculatesMeanVarianceAndMoments()
    {
        ...
        var meanVariance = DescriptiveStatistics.MeanVariance(Values);
        var moments = DescriptiveStatistics.Moments(Values);
    ...
    public void Statistics_CalculatesCovarianceSummary()
    {
        ...
        var statistics = DescriptiveStatistics.Covariance(x, y);
    ...
    {
        DescriptiveStatistics.Mean([]).ShouldBe(double.NaN);

        var meanVariance = DescriptiveStatistics.MeanVariance([]);
        var moments = DescriptiveStatistics.Moments([]);
        var covariance = DescriptiveStatistics.Covariance([], []);
        ...
        Should.Throw<ArgumentException>(() => DescriptiveStatistics.Covariance([1.0], [1.0, 2.0]));
    }

    [Fact]
    public void Statistics_ReportsUndefinedSampleStatisticsForInsufficientObservations()
    {
        var singleVariance = DescriptiveStatistics.MeanVariance([42.0]);
        var singleMoments = DescriptiveStatistics.Moments([42.0]);
        var pairedSingle = DescriptiveStatistics.Covariance([2.0], [3.0]);
        var twoMoments = DescriptiveStatistics.Moments([1.0, 2.0]);
