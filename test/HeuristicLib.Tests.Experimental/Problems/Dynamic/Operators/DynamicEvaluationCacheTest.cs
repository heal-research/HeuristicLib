...
file sealed record CountingEvaluator : StatelessEvaluator<DummyGenotype, DummySearchSpace, DummyDynamicProblem>
{
    ...
    public override IReadOnlyList<EvaluatedCandidate<DummyGenotype>> Evaluate(
      IReadOnlyList<DummyGenotype> candidates,
      IRandomNumberGenerator random,
      DummySearchSpace searchSpace,
      DummyDynamicProblem problem)
    {
        Calls++;
        LastBatchSize = candidates.Count;

        return candidates.Select(candidate => candidate.ToEvaluated(problem.Evaluate(candidate, random))).ToArray();
    }
}
...
public class DynamicEvaluationCacheTests
{
    ...
    public void CachesAcrossBatches_NoReevaluation_NoTick()
    {
        ...
        var cached = new ExecutionInstanceRegistry().Resolve(new DynamicCachingEvaluator<DummyGenotype, DummySearchSpace, DummyDynamicProblem, int>(
          inner, problem, keySelector: s => s.Value
        ));
