...
{
    public QualityCurvePerEpochAnalysis(TProblem problem,
                                        params IReadOnlyList<IEvaluator<TCandidate, TSearchSpace, TProblem>>
                                            evaluators) : base(problem, evaluators)
    { }

    public override QualityCurvePerEpochAnalysisResult<TCandidate> CreateInitialResult() =>
        new(Problem.Objective);
}
...
{
    private readonly List<(TCandidate candidate, ObjectiveVector objectiveVector, EvaluationTiming timing)>
        bestPerEpoch = [];

    public IReadOnlyList<(TCandidate candidate, ObjectiveVector objectiveVector, EvaluationTiming timing)>
        BestPerEpoch => bestPerEpoch;
                                       ...
                                       EvaluationTiming timing)> evaluationLog)
    {
        foreach (var e in evaluationLog.Where(x => x.timing.Valid))
        {
            if (bestPerEpoch.Count > 0)
            {
                ...
                if (best.timing.Epoch == e.timing.Epoch &&
                    objective.TotalOrderComparer.Compare(best.objectiveVector, e.objective) >= 0)
                    continue;
            }
