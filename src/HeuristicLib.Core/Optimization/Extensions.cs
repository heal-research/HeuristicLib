...
public static class Extensions
{
    ...
    ParallelOptions? parallelOptions = null)
  {
    ...
    if (parallelOptions == null) {
      Parallel.For(0, source.Count, body: i => {
      ...
      Parallel.For(0, source.Count, parallelOptions, body: i => {
        selected[i] = action(i, source[i], randoms[i]);
      });
    }
  ...
  public static T1[] GetMinBy<T1, T2>(
    ...
    =>
      source.GetMinBy(selector, Comparer<T2>.Default, m);
  ...
  public static int Count(this Range r) => r.End.Value - r.Start.Value;
