...
public static class RandomEnumerable
{
  ...
  extension<T>(IEnumerable<T> source)
  {
    ...
    public T SampleProportional(IRandomNumberGenerator random, IEnumerable<double> weights) => source.SampleProportional(random, weights, false, false).First();

    public T SampleProportional(IEnumerable<double> weights, IRandomNumberGenerator random) => source.SampleProportional(random, weights);

    /// <summary>
    ///   Chooses elements out of a sequence with repetition. The chance that an item is selected is proportional or
    ...
    /// <returns>
    ///   A sequence of selected items. Might actually be shorter than <paramref name="count" /> elements if source has
    ///   less than <paramref name="count" /> elements.
    /// </returns>
    public IEnumerable<T> SampleProportionalWithoutRepetition(IRandomNumberGenerator random, int count, IEnumerable<double> weights, bool windowing = true, bool inverseProportional = false) => source.SampleProportionalWithoutRepetition(random, weights, windowing, inverseProportional).Take(count);
  }
    ...
    if (minValue.IsAlmost(maxValue)) {
      ...
      if (windowing) {
        if (inverseProportional) {
          InverseProportionalScale(valueArray, maxValue);
        } else {
          ProportionalScale(valueArray, minValue);
        }
      } else {
        if (minValue < 0.0) {
          throw new InvalidOperationException("Proportional selection without windowing does not work with values < 0.");
        }

        if (inverseProportional) {
          InverseProportionalScale(valueArray, 2 * maxValue);
        }
      }
    }
  ...
  /// <param name="random">
  ///   The random number generator, its Next(n) method must deliver uniformly distributed random numbers
  ///   in the range [0;n).
  /// </param>
