...
public static class IterationObserver
{
  public static IIterationObserver<TG, TS, TP, TR> Create<TG, TS, TP, TR>(Action<TR, TR?, TS, TP> onIterationCompleted)
    ...
    new FuncIterationObserver<TG, TS, TP, TR>(onIterationCompleted);
}
  ...
  where TR : class, IAlgorithmState
{
  ...
  void IIterationObserver<TG, TS, IProblem<TG, TS>, TR>.OnIterationCompleted(TR currentState, TR? previousState, TS searchSpace, IProblem<TG, TS> problem) => OnIterationCompleted(currentState, previousState, searchSpace);
}
  ...
  where TR : class, IAlgorithmState
{
  ...
  void IIterationObserver<TG, ISearchSpace<TG>, IProblem<TG, ISearchSpace<TG>>, TR>.OnIterationCompleted(TR currentState, TR? previousState, ISearchSpace<TG> searchSpace, IProblem<TG, ISearchSpace<TG>> problem) => OnIterationCompleted(currentState, previousState);
}
...
public sealed class FuncIterationObserver<TG, TS, TP, TR>(Action<TR, TR?, TS, TP> onIterationCompleted) : IterationObserver<TG, TS, TP, TR>
  ...
  where TR : class, IAlgorithmState
{
  public override void OnIterationCompleted(TR currentState, TR? previousState, TS searchSpace, TP problem) => onIterationCompleted(currentState, previousState, searchSpace, problem);
}
