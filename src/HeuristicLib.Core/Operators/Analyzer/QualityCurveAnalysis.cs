...
public class QualityCurveAnalysis<TGenotype> : AttachedAnalysis<TGenotype> where TGenotype : class {
  ...
  public override void AfterEvaluation(IReadOnlyList<TGenotype> genotypes, IReadOnlyList<ObjectiveVector> values, IEncoding<TGenotype> searchSpace, IProblem<TGenotype, IEncoding<TGenotype>> problem) {
