...
public class GenealogyGraph<TGenotype> where TGenotype : notnull
{
  ...
  public class Node(int id, TGenotype value, int generation, int layer, int rank)
  {
    ...
    public HashSet<Node> GetAllDescendants() => Children.SelectMany(x => x.GetAllDescendants()).Append(Children).ToHashSet();

    public HashSet<Node> GetAllAncestors() => Parents.SelectMany(x => x.GetAllAncestors()).Append(Children).ToHashSet();
  }
}
