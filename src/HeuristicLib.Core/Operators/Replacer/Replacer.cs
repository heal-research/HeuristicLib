...
public abstract class Replacer<TGenotype> : IReplacer<TGenotype> {
  ...
  IReadOnlyList<ISolution<TGenotype>> IReplacer<TGenotype, IEncoding<TGenotype>, IProblem<TGenotype, IEncoding<TGenotype>>>.Replace(IReadOnlyList<ISolution<TGenotype>> previousPopulation, IReadOnlyList<ISolution<TGenotype>> offspringPopulation, Objective objective, IRandomNumberGenerator random, IEncoding<TGenotype> searchSpace, IProblem<TGenotype, IEncoding<TGenotype>> problem) {
    return Replace(previousPopulation, offspringPopulation, objective, random);
  }

  IReadOnlyList<ISolution<TGenotype>> IReplacer<TGenotype, IEncoding<TGenotype>>.Replace(IReadOnlyList<ISolution<TGenotype>> previousPopulation, IReadOnlyList<ISolution<TGenotype>> offspringPopulation, Objective objective, IRandomNumberGenerator random, IEncoding<TGenotype> searchSpace) {
    return Replace(previousPopulation, offspringPopulation, objective, random);
  }
}
