  ...
  private sealed class ObservableCreatorInstance(ICreatorInstance<TG, TS, TP> creatorInstance, IReadOnlyList<ICreatorObserver<TG, TS, TP>> observers) : CreatorInstance<TG, TS, TP>
  {
    public override IReadOnlyList<TG> Create(int count, IRandomNumberGenerator random, TS searchSpace, TP problem)  {
      ...
      foreach (var observer in observers) {
        observer.AfterCreation(result, searchSpace, problem);
      }
...
{
  void AfterCreation(IReadOnlyList<T> offspring, TS searchSpace, TP problem);
}
...
public class CreatorObserver<TG, TS, TP> : ICreatorObserver<TG, TS, TP>
  ...
  where TP : class, IProblem<TG, TS>
{
  private readonly Action<IReadOnlyList<TG>, TS, TP> afterCreation;
  
  public CreatorObserver(Action<IReadOnlyList<TG>, TS, TP> afterCreation)
  {
    this.afterCreation = afterCreation;
  }
  
  public void AfterCreation(IReadOnlyList<TG> offspring, TS searchSpace, TP problem)
  {
    afterCreation.Invoke(offspring, searchSpace, problem);
  }
}
...
public static class ObservableCreatorExtensions
{
    ...
    where TP : class, IProblem<TG, TS>
  {
    ...
    public ICreator<TG, TS, TP> ObserveWith(Action<IReadOnlyList<TG>, TS, TP> afterCreation)
    {
    ...
    {
      var observer = new CreatorObserver<TG, TS, TP>((offspring, _, _) => afterCreation(offspring));
      return creator.ObserveWith(observer);
    }
