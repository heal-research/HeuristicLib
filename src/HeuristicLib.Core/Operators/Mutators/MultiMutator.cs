  ...
  where TProblem : class, IProblem<TGenotype, TSearchSpace>
{
  ...
  public override IReadOnlyList<TGenotype> Mutate(IReadOnlyList<TGenotype> parent, IRandomNumberGenerator random, TSearchSpace searchSpace, TProblem problem)
  {
    ...
    for (var i = 0; i < offspringCount; i++) {
      ...
      if (idx < 0) {
        idx = Mutators.Count - 1; // defensive fallback
      }
...
public class MultiMutator<TGenotype, TSearchSpace> : MultiMutator<TGenotype, TSearchSpace, IProblem<TGenotype, TSearchSpace>>
  where TSearchSpace : class, ISearchSpace<TGenotype>
{
  public MultiMutator(IReadOnlyList<IMutator<TGenotype, TSearchSpace, IProblem<TGenotype, TSearchSpace>>> mutators, IReadOnlyList<double>? weights = null)
    : base(mutators, weights)
  {
  }
...
{
  public MultiMutator(IReadOnlyList<IMutator<TGenotype, ISearchSpace<TGenotype>, IProblem<TGenotype, ISearchSpace<TGenotype>>>> mutators, IReadOnlyList<double>? weights = null)
    : base(mutators, weights)
  {
  }
