...
public class SimulatedBinaryCrossover : Crossover<RealVector, RealVectorSearchSpace>
{
  ...
  /// </exception>
  /// <remarks>
  ///   The manipulated value is not restricted by the (possibly) specified lower and upper bounds. Use the <see /> to
  ///   correct the values after performing the crossover.
  /// </remarks>
  ...
  protected RealVector Cross(IRandomNumberGenerator random, RealVector[] parents) => parents.Length == 2 ? Apply(random, parents[0], parents[1], Contiguity) : throw new ArgumentException("SimulatedBinaryCrossover: The number of parents is not equal to 2");
