  ...
  where TProblem : class, IProblem<TGenotype, TSearchSpace>
{
  ...
  public IReadOnlyList<ObjectiveVector> Apply(IReadOnlyList<TGenotype> input, IOptimizationContext<TGenotype, TSearchSpace, TProblem> context) =>
  ...
  where TSearchSpace : class, ISearchSpace<TGenotype>
{
  ...
  public IReadOnlyList<ObjectiveVector> Apply(IReadOnlyList<TGenotype> input, IOptimizationContext<TGenotype, TSearchSpace, IProblem<TGenotype, TSearchSpace>> context) => Evaluate(input, context.Random, context.SearchSpace);
}
...
public abstract class Evaluator<TGenotype> : IEvaluator<TGenotype, ISearchSpace<TGenotype>, IProblem<TGenotype, ISearchSpace<TGenotype>>>
{
  ...
  public IReadOnlyList<ObjectiveVector> Apply(IReadOnlyList<TGenotype> input, IOptimizationContext<TGenotype, ISearchSpace<TGenotype>, IProblem<TGenotype, ISearchSpace<TGenotype>>> context) => Evaluate(input, context.Random);
}
  ...
  where TP : class, IProblem<TG, TS>
{
  ...
  public override IReadOnlyList<ObjectiveVector> Evaluate(IReadOnlyList<TG> solutions, IRandomNumberGenerator random, TS searchSpace, TP problem)
  {
    ...
    return @operator.Apply(solutions, context);
  }
}
