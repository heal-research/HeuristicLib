...
public static class BestSelector
{
  public static int Select(IReadOnlyList<ObjectiveVector> population, Objective objective, IRandomNumberGenerator random)
  {
    return population.Count switch {
      ...
      _ => Enumerable.Range(0, population.Count).MinBy(keySelector: x => population[x], objective.TotalOrderComparer)
    };
  }
  ...
  public static IReadOnlyList<int> Select(IReadOnlyList<ObjectiveVector> population, Objective objective, int count, IRandomNumberGenerator random)
  {
    ...
    return count == 1
      ? [Enumerable.Range(0, population.Count).MinBy(keySelector: x => population[x], objective.TotalOrderComparer)]
      : Enumerable.Range(0, population.Count).GetMinBy(selector: x => population[x], objective.TotalOrderComparer, count);
  }
}
...
public class BestSelector<TGenotype> : BatchSelector<TGenotype>
{
  ...
  {
    return count == 1
      ? [population.MinBy(keySelector: x => x.ObjectiveVector, objective.TotalOrderComparer)!]
      : population.GetMinBy(selector: x => x.ObjectiveVector, objective.TotalOrderComparer, count);
  }
}
