...
public class RemoveDuplicatesInterceptor<TGenotype, TIterationResult>(IEqualityComparer<TGenotype> comparer) : Interceptor<TGenotype, TIterationResult>
  where TIterationResult : IPopulationAlgorithmState<TGenotype, TIterationResult>
{
  ...
  {
    var newISolutions = currentIterationResult.Solutions.DistinctBy(keySelector: s => s.Genotype, comparer);
