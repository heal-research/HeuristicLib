...
public abstract class Algorithm<TGenotype, TEncoding, TProblem, TAlgorithmResult> : IAlgorithm<TGenotype, TEncoding, TProblem, TAlgorithmResult>
  ...
  IAlgorithmState IAlgorithm<TGenotype, TEncoding, TProblem>.Execute(TProblem problem, TEncoding? searchSpace, IRandomNumberGenerator? random) => Execute(problem, searchSpace, random);
}
