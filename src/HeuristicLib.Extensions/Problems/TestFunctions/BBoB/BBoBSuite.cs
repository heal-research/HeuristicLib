...
public static class BBoBSuite
{
  ...
  private static RastriginFunction CreateRastriginProblem(int function, int dimension, int instance, long randomSeed) => new() { Dimension = dimension };

  private static BuecheRastriginFunction CreateBuecheRastriginProblem(int function, int dimension, int instance, long randomSeed3) => new() { Dimension = dimension };

  private static LinearSlopeFunction CreateLinearSlopeProblem(int function, int dimension, int instance, long randomSeed) => new(CreateXOpt(dimension, randomSeed)) { Dimension = dimension };
  ...
  private static RosenbrockFunction CreateRosenbrockProblem(int function, int dimension, int instance, long randomSeed)
  {
  ...
  private static DiscusFunction CreateDiscusProblem(int function, int dimension, int instance, long randomSeed) => new() { Dimension = dimension };

  private static BentCigarFunction CreateBentCigarProblem(int function, int dimension, int instance, long randomSeed) => new() { Dimension = dimension };

  private static SharpRidgeFunction CreateSharpRidgeProblem(int function, int dimension, int instance, long randomSeed) => new() { Dimension = dimension };

  private static DifferentPowersFunction CreateDifferentPowersProblem(int function, int dimension, int instance, long randomSeed) => new() { Dimension = dimension };

  private static RastriginFunction CreateRastriginRotatedProblem(int function, int dimension, int instance, long randomSeed)
  {
    // BBOB rotated Rastrigin.
    return new RastriginFunction { Dimension = dimension };
  }

  private static WeierstrassFunction CreateWeierstrassProblem(int function, int dimension, int instance, long randomSeed)
  {
  ...
  private static SchaffersFunction CreateSchaffersProblem(int function, int dimension, int instance, long randomSeed, int condition)
  {
  ...
  private static GriewankRosenbrockFunction CreateGriewankRosenbrockProblem(int function, int dimension, int instance, long randomSeed) => new() { Dimension = dimension };

  private static SchwefelFunction CreateSchwefelProblem(int function, int dimension, int instance, long randomSeed) => new() { Dimension = dimension };

  private static GallagherFunction CreateGallagherProblem(int function, int dimension, int instance, long randomSeed, int numberOfPeaks)
  {
    ...
    for (var i = 0; i < numberOfPeaks - 1; i++) {
      rpermPeaks[i] = new FGallagherPermutation(randomNumbers[i], i);
    }

    Array.Sort(rpermPeaks, comparison: (a, b2) => a.Value.CompareTo(b2.Value));
    ...
    for (var i = 0; i < numberOfPeaks; i++) {
      ...
      for (var j = 0; j < dimension; j++) {
        rpermDims[j] = new FGallagherPermutation(randomNumbers[j], j);
      }

      Array.Sort(rpermDims, comparison: (a, b2) => a.Value.CompareTo(b2.Value));
  ...
  private static KatsuuraFunction CreateKatsuuraProblem(int function, int dimension, int instance, long randomSeed) => new() { Dimension = dimension };

  private static LunacekBiRastriginFunction CreateLunacekBiRastriginProblem(
    int function,
    int dimension,
    int instance,
    long rseed)
...
#pragma warning restore S1244
  ...
  {
    long randomSeed = function switch {
    ...
    var rrseed = randomSeed + 10000L * instance;
