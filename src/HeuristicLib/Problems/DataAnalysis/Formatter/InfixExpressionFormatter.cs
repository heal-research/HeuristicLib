            ...
            case 0 when node is VariableTreeNode varNode:
                {
                    ...
                    { throw new NotSupportedException("Infix formatter does not support variables with coefficients."); }
            ...
            case > 1:
                {
                    ...
                    if (IsOperator(node.Symbol))
                    {
                        ...
                        FormatRecursively(node.Subtrees[0], strBuilder, numberFormat, formatString, parameters);
