...
public static class CrossoverObserver {
  public static ICrossoverObserver<TG, TS, TP> Create<TG, TS, TP>(Action<IReadOnlyList<IParents<TG>>, IReadOnlyList<TG>, TS, TP> onCrossoverCompleted)
    ...
    where TS : class, ISearchSpace<TG>
    where TP : class, IProblem<TG, TS> {
    return new FuncCrossoverObserver<TG, TS, TP>(onCrossoverCompleted);
  }
}
  ...
  where TS : class, ISearchSpace<TG>
{
  ...
  public void OnCrossoverCompleted(IReadOnlyList<IParents<TG>> parents, IReadOnlyList<TG> offspring, TS searchSpace, IProblem<TG, TS> problem) {
    OnCrossoverCompleted(parents, offspring, searchSpace);
  }
}
  ...
  where TG : class
{
  ...
  public void OnCrossoverCompleted(IReadOnlyList<IParents<TG>> parents, IReadOnlyList<TG> offspring, ISearchSpace<TG> searchSpace, IProblem<TG, ISearchSpace<TG>> problem) {
    OnCrossoverCompleted(parents, offspring);
  }
}
...
public sealed class FuncCrossoverObserver<TG, TS, TP>(Action<IReadOnlyList<IParents<TG>>, IReadOnlyList<TG>, TS, TP> onCrossoverCompleted) : CrossoverObserver<TG, TS, TP>
  ...
  where TP : class, IProblem<TG, TS>
{
  public override void OnCrossoverCompleted(IReadOnlyList<IParents<TG>> parents, IReadOnlyList<TG> offspring, TS searchSpace, TP problem) {
    onCrossoverCompleted(parents, offspring, searchSpace, problem);
  }
}
