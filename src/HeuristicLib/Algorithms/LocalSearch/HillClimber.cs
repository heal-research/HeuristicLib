    ...
    where TProblem : class, IProblem<TGenotype, TSearchSpace>
{
        ...
        IRandomNumberGenerator random)
    {
        if (previousState is null)
        {
            ...
            return new SingleSolutionState<TGenotype> { Population = Population.From([initialSolution], [initialFitness]) };
        }
        ...
        for (var i = 0; i < MaxNeighbors; i += BatchSize)
        {
            ...
            var best = BestSelector.Select(res.Append(sol.ObjectiveVector).ToArray(), problem.Objective)[0];
            if (best == BatchSize)
            {
                continue;
            }
            ...
            {
                return new SingleSolutionState<TGenotype> { Population = Population.From([newISolution.Genotype], [newISolution.ObjectiveVector]) };
            }
        }
