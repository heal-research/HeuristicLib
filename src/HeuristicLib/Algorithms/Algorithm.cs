...
{
  TResultState Execute(TStartState? startState = default);
}
...
{
  TResultState Execute(TContinuationState continuationState);
}

public static class AlgorithmExtensions {
  public static IEnumerable<TResultState> CreateResultStream
    <TStartState, TContinuationState, TResultState>
    (this IContinuableAlgorithm<TStartState, TContinuationState, TResultState> algorithm, TStartState? startState = default, ITerminator<TResultState>? terminator = null)
    where TStartState : IStartState 
    where TContinuationState : IContinuationState
    where TResultState : IContinuableResultState<TContinuationState>
  {
    var currentResult = algorithm.Execute(startState);
    yield return currentResult;
    while (terminator?.ShouldContinue(currentResult) ?? true) {
      var continuationState = currentResult.GetNextContinuationState();
      currentResult = algorithm.Execute(continuationState);
      yield return currentResult;
    }
  }
  public static IEnumerable<TResultState> CreateResultStream
    <TStartState, TContinuationState, TResultState>
    (this IContinuableAlgorithm<TStartState, TContinuationState, TResultState> algorithm, TContinuationState continuationState, ITerminator<TResultState>? terminator = null)
    where TStartState : IStartState 
    where TContinuationState : IContinuationState
    where TResultState : IContinuableResultState<TContinuationState>
  {
    ...
    do {
      currentResult = algorithm.Execute(currentContinuationState);
      yield return currentResult;
  ...
  public static TResultState Execute
    <TStartState, TContinuationState, TResultState>
    (this IContinuableAlgorithm<TStartState, TContinuationState, TResultState> algorithm, ITerminator<TResultState> terminator, TStartState? startState = default)
  ...
  {
    return algorithm.CreateResultStream(startState, terminator).Last();
  }
  public static TResultState Execute
    <TStartState, TContinuationState, TResultState>
    (this IContinuableAlgorithm<TStartState, TContinuationState, TResultState> algorithm, ITerminator<TResultState> terminator, TContinuationState continuationState)
    where TStartState : IStartState where TContinuationState : IContinuationState where TResultState : IContinuableResultState<TContinuationState>
  {
    return algorithm.CreateResultStream(continuationState, terminator).Last();
  }
}

public abstract class AlgorithmBase<TStartState, TResultState> 
  : IAlgorithm<TStartState, TResultState>
  where TStartState : IStartState
  where TResultState : IResultState
{
  public abstract TResultState Execute(TStartState? startState = default);
}

public abstract class AlgorithmBase<TStartState, TContinuationState, TResultState> 
  : AlgorithmBase<TStartState, TResultState>, 
    IContinuableAlgorithm<TStartState, TContinuationState, TResultState>
  where TStartState : IStartState 
  where TContinuationState : IContinuationState 
  where TResultState : IContinuableResultState<TContinuationState>
{
  public abstract TResultState Execute(TContinuationState continuationState);
}
