...
public interface IEncoding<in TGenotype, in TSelf> : IEncoding where TSelf : IEncoding<TGenotype, TSelf> {
  bool IsValidGenotype(TGenotype genotype);
  //bool IsValidOperator(IEncodingOperator<TGenotype, TSelf> @operator);
}

public abstract record class EncodingBase<TGenotype, TSelf> : IEncoding<TGenotype, TSelf>
  where TSelf : EncodingBase<TGenotype, TSelf> {
