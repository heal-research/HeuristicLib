...
public class IntegerVector(params IEnumerable<int> elements) : IReadOnlyList<int>, IEquatable<IntegerVector> {
  ...
  public static implicit operator RealVector.RealVector(IntegerVector integerVector) {
    return new RealVector.RealVector(integerVector.elements.Select(i => (double)i));
  }
}
