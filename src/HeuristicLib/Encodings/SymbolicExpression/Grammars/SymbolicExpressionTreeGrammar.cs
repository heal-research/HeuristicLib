...
namespace HEAL.HeuristicLib.Operators.SymbolicExpression.Grammars {
  ...
  internal sealed class SymbolicExpressionTreeGrammar : SymbolicExpressionGrammarBase, ISymbolicExpressionTreeGrammar {
    ...
    [Storable] private readonly ISymbolicExpressionGrammar grammar;

    public SymbolicExpressionTreeGrammar(ISymbolicExpressionGrammar grammar)
      : base(nameof(SymbolicExpressionTreeGrammar), "A grammar that is used held by symbolic expression trees and allows extensions to the wrapped grammar.") {
    ...
    public override IEnumerable<ISymbol> Symbols {
      get {
        ...
        foreach (var s in symbols.Values) yield return s;
      }
    }
    public IEnumerable<ISymbol> ModifyableSymbols => symbols.Values;
