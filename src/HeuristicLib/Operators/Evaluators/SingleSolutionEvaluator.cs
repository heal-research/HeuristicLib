  ...
  where TProblem : class, IProblem<TCandidate, TSearchSpace>
{
    ...
    public override IReadOnlyList<EvaluatedCandidate<TCandidate>> Evaluate(IReadOnlyList<TCandidate> candidates, IRandomNumberGenerator random, TSearchSpace searchSpace, TProblem problem) =>
      BatchExecution.Parallel(candidates, (candidate, r) => candidate.ToEvaluated(Evaluate(candidate, r, searchSpace, problem)), random, MaxDegreeOfParallelism);
}
  ...
  where TSearchSpace : class, ISearchSpace<TCandidate>
{
    ...
    public override IReadOnlyList<EvaluatedCandidate<TCandidate>> Evaluate(IReadOnlyList<TCandidate> candidates, IRandomNumberGenerator random, TSearchSpace searchSpace) =>
      BatchExecution.Parallel(candidates, (candidate, r) => candidate.ToEvaluated(Evaluate(candidate, r, searchSpace)), random, MaxDegreeOfParallelism);
}
  ...
  : StatelessEvaluator<TCandidate>
{
    ...
    public override IReadOnlyList<EvaluatedCandidate<TCandidate>> Evaluate(IReadOnlyList<TCandidate> candidates, IRandomNumberGenerator random) =>
      BatchExecution.Parallel(candidates, (candidate, r) => candidate.ToEvaluated(Evaluate(candidate, r)), random, MaxDegreeOfParallelism);
}
