...
- Do not require ordinary users to manually wire the counted wrapper and the matching terminator as separate objects for common operator-budget cases. Typed helpers such as `WithMaxEvaluatorCalls(...)`, `WithMaxMutatorCalls(...)`, and `WithMaxSelectedSolutions(...)` perform the correct replacement and attach the corresponding external early-stopping policy. `WithMaxCount(...)` remains the lower-level factory shape for custom counted wrappers or unusual operator boundaries.
- Keep `WithMaxCount(...)` as the general operator-budget escape hatch. Typed helpers exist for the regular operator families so callers do not need to write factory lambdas for ordinary count budgets.
...
| Selector | [x] `CountSelectorCalls(...)`, `CountSelectedSolutions(...)` | [x] `WithMaxSelectorCalls(selector, ...)`, `WithMaxSelectedSolutions(selector, ...)` | [x] `MeasureSelectorDuration(...)` plus wrapper | [x] `WithMaxSelectorDuration(selector, ...)` |
| Replacer | [x] `CountReplacerCalls(...)`, `CountReplacementSolutions(...)` | [x] `WithMaxReplacerCalls(replacer, ...)`, `WithMaxReplacementSolutions(replacer, ...)` | [x] `MeasureReplacerDuration(...)` plus wrapper | [x] `WithMaxReplacerDuration(replacer, ...)` |
| Interceptor | [x] `CountInterceptorCalls(...)` | [x] `WithMaxInterceptorCalls(interceptor, ...)` | [x] `MeasureInterceptorDuration(...)` plus wrapper | [x] `WithMaxInterceptorDuration(interceptor, ...)` |
| Terminator | [x] `CountTerminatorCalls(...)` | [x] `WithMaxTerminatorCalls(terminator, ...)` | [x] `MeasureTerminatorDuration(...)` plus wrapper | [x] `WithMaxTerminatorDuration(terminator, ...)` |
