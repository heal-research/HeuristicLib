...
| Evaluator contract | Settled: problem evaluation stays scalar and pure, `ProblemEvaluator` is the default terminal evaluator that parallelizes problem calls, and evaluator operators return the authoritative evaluated solution. Numeric-parameter optimization is evaluator behavior, not a separate refiner operator. |
