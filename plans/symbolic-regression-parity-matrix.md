...
| Evolvable-constant optimization | Levenberg-Marquardt parameter optimization | Planned reusable differentiation, numerical optimization, and memetic composition | Reference prototype archived | The replacement is specified in `symbolic-regression-constant-optimization-plan.md`; the first direct solver remains available only as reference material. |
...
| Legacy migration | Incremental replacement: a source snapshot is retained under `references/symbolic-regression/legacy-mutable-system`, while still-required mutable components stay in their domain folders and are removed only after replacement coverage and explicit approval. `.Legacy` namespaces are used only where old and new public type names would otherwise collide. |
