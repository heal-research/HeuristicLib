...
| Refiner hook | Settled: algorithm-level pre-evaluation refiner is a problem-typed operator whose general call receives candidates, random source, search space, and problem; symbolic-regression refiners extract input variables, target data, interpreter, and bounds from the typed problem instead of accepting those details as ad hoc `Refine` parameters. |
| Refiner factory API | Settled direction: expose common symbolic-expression refiners through `SymbolicExpressionRefiner`, for example `SymbolicExpressionRefiner.OptimizeNumericParameters(...)`; Levenberg-Marquardt can be the first implementation without being required in the common authoring name. |
