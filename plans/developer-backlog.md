...
- reduce the generic arity a user has to name, per [generic-arity-usability.md](generic-arity-usability.md). That plan holds the measurements, the settled decisions on `TProblem` and on when a search space may be split, and the open question of whether `TSearchSpace` can leave the configuration layer while staying on execution instances
