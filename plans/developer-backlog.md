...
- design sensible creator, crossover and mutator defaults for the remaining encodings and for problems with domain-specific recommendations. `PermutationSearchSpace` and `TravelingSalesmanProblem` prove the defaults mechanism and are enough for the current ergonomics work. Do not roll defaults out mechanically. Each choice needs evidence that it is a useful starting point for that encoding or problem, plus documentation that taking a default affects behavior and reproducibility
