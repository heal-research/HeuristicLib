...
- (settled and done) the resolution attempt is gone from `Validate` and `ValidateAndThrow`. Both questions are now answered from declarations: the invariant contracts, and `IExecutionInstanceResolvable.Fits`, which decides from type arguments what the authoring bases decide when they bridge. See step 2 of [generic-arity-usability.md](generic-arity-usability.md)
...
- design sensible creator, crossover and mutator defaults for the remaining encodings and for problems with domain-specific recommendations. `PermutationSearchSpace` and `TravelingSalesmanProblem` prove the defaults mechanism and are enough for the current ergonomics work. Do not roll defaults out mechanically. Each choice needs evidence that it is a useful starting point for that encoding or problem, plus documentation that taking a default affects behavior and reproducibility. Decide at the same time whether reproducibility metadata should record which operators were defaulted rather than chosen, since a default is public API and changing it changes results for everyone who took it
- finish the remaining ergonomics work from [generic-arity-usability.md](generic-arity-usability.md). **The arity migration itself is done** — every exit criterion in [configuration-arity-migration.md](configuration-arity-migration.md) is met, all nine role contracts name only the candidate, `RoleContractArityTests` guards that shape, and the documentation and specs reflect it. Of that document's four formerly open steps, 4 and 6 closed with the migration and 5 was superseded by it. **Only step 1 remains**: declaring operator defaults, which is what makes `For(problem, …)` available at all. Its two halves are independent. The problem half is not a defaults question — every `IProblemDefault*` member is a `static virtual` returning `null`, so a problem must declare three interfaces at full triple arity to add nothing, and the `static` is what forces the constraint. Making those members instance members at candidate arity would remove the constraint instead of relocating it; having the `Problem` base declare the interfaces is rejected, because a base class naming the built-in default interfaces privileges built-in roles over a consumer's. The search space half is the real defaults decision and stays coupled to the item below, which requires evidence per choice rather than mechanical rollout
...
- move to a more standard C# formatting baseline, including a more conventional editor configuration and CI enforcement that fails pull requests on formatting violations. The rules settled while cleaning up the arity branch are: the expression-body arrow ends the previous line; block indentation is always a multiple of four, while column alignment is left alone; a `where` constraint aligns with the base list, both at declaration + 4; a continuation indents four from the line that opened it rather than aligning to a column, and a wrapped construct breaks at its first element; a parameter list breaks after the opening parenthesis or not at all, never between parameters. Two decisions were deliberately deferred and should be taken with this item. First, whether a multi-line parameter list should close with `)` on its own line, which would make the start of a `where` block easier to see, rather than attached to the last parameter as it is now. Second, whether the roughly 990 remaining mid-expression continuation lines that sit at an indent of `4n + 2` should be normalized; the declaration-level cases are already fixed. Note that none of these rules is enforced by anything today: `dotnet format` accepts every variant, and Rider normalizes some of them but also rewraps long signatures in a way this repository does not follow, so reconciling `.editorconfig` with Rider's wrapping settings is part of this item
...
- compile the documentation snippets. Nothing builds them, so a guide page can contradict the library indefinitely: the arity branch found broken snippets on six pages, two of which had never compiled in any form, and the sixth surfaced only after the migration was believed complete — a full-arity `ChooseOneMutator` in the symbolic regression guide, found by grepping for the shape rather than by anything that builds. Every fix there was verified by pasting the snippet into `HeuristicLib.Tests.ApiUsageSpecs`, building, and deleting the probe — the right mechanism run as a manual process that nothing repeats. Decide between extracting fenced `csharp` blocks into a generated compile-only project and keeping the canonical snippets in `ApiUsageSpecs` and including them into the docs, then apply it to the guide pages and `README.md`
...
- migrate HeuristicLab operators and their unit tests in a structured way; track operator mappings, missing operators, test migration status, and intentional behavior/API differences in the [HeuristicLab operator migration overview](heuristiclab_operator_migration_overview.md)
...
- decide whether execution needs a problem dependent validation phase after the execution graph is resolved and before algorithm work begins. `NumericParameterFittingRefiner` is the first concrete case: enabling numeric parameter fitting while the symbolic expression search space allows operations the differentiable compiler cannot lower should fail before candidate creation, random draws or evaluation. A future design must find the right validation owner, reach nested operators and delayed child execution graphs, preserve configuration and execution instance ownership and avoid adding a broad operator lifecycle hook solely for this one case. Until then the refiner reports the incompatibility when it encounters an affected candidate
- decide how cancellation reaches operators. No operator role signature carries a `CancellationToken`, so an operator wrapping a long-running computation cannot observe cancellation and cannot pass one to a component that accepts it. `NumericParameterFittingRefiner` is the first concrete case: `NumericParameterFitter` takes a token and checks it inside the Levenberg-Marquardt solve, but the refiner has none to give and passes `default`, so a solve over a large population cannot be interrupted. Decide whether cancellation belongs in the role signatures, in a run-scoped execution context reaching instances through the execution registry, or on the execution instance itself; the answer interacts with the separate run-level execution-policy item above, since both concern run-scoped state that must reach operators without ambient statics. Consider that most operators are short and would only forward a token they never check
- find a way to reduce the operator boilerplate that a role-contract change has to touch. The problem is unsolved, and both approaches tried so far are recorded in [Discussed, tried, and rejected](#discussed-tried-and-rejected): [typed operator invocation](#typed-operator-invocation) and [generated operator families](#generated-operator-families). Do not re-propose either without the new evidence those entries require.
  ...
  The evaluator contract change from `EvaluatedCandidate<TCandidate>` to `ObjectiveVector` (RF-2 of the symbolic-regression parameter-fitting work; see [symbolic-regression decisions](symbolic-regression-decisions.md)) is the clearest measurement available so far. It changed the single most fundamental property of an operator role — what its execution instance returns — and is worth reading from two directions at once, because the pair is the actual finding.
...
Unlike the open items above, these are settled. Rules that follow from them live in [developer guidelines](../docs/contributing/developer-guidelines.md) and the relevant topic pages; this section holds only the reasoning.
...
### Algorithm construction through builders, problem witnesses and value-dependent defaults

Settled during the API ergonomics work in August 2026, after each variant was prototyped against the real library.

`GeneticAlgorithmBuilder` and its siblings were phased out rather than polished into a first-class API. The record plus a static companion covers every case the builders did: `Create(...)` where the caller supplies the required operators, `For(anchor, ...)` where a problem or encoding supplies both the type arguments and the defaults, and `with` for incremental changes. What the builders offered that nothing replaces is mutable statement-by-statement configuration spread across a method body; porting the call sites made them shorter, not longer.

Passing the problem to `Create(...)` purely as a type witness was measured and dropped. With all-generic operators and no problem argument, inference already yields `GeneticAlgorithm<Permutation, PermutationSearchSpace, IProblem<…>>`, and that algorithm runs against a concrete problem; adding the problem produces the identical type while a problem-bound operator pins `TProblem` exactly on its own. The parameter was inert, and the compiler reports it as such.

Value-dependent defaults were kept as a capability and rejected as a shipped behavior. The interfaces take the instance, so a default may depend on it, but a rule such as "length under twenty picks a swap mutator, otherwise an inversion mutator" would publish an arbitrary threshold as public API.

Reopening any of these needs a concrete case the current shape cannot express: a configuration that genuinely requires mutation across many statements, an inference failure that only an extra witness parameter fixes, or a value-dependent default whose threshold is derived rather than chosen.
...
Operator families are therefore ordinary checked-in C# source; see [operator implementation](../docs/contributing/architecture/operator-implementation.md#scaffolding-roles-and-cross-cutting-concerns) for the workflow and its guardrails.
...
- [design-goals.md](../docs/contributing/design-goals.md)
- [developer-guidelines.md](../docs/contributing/developer-guidelines.md)
- [requirements.md](../docs/contributing/requirements.md)
