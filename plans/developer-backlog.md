...
- define a concrete namespace and source-layout ergonomics target from real examples: reduce the `using` lists in `docs/guide/getting-started.md`, `docs/guide/index.md`, and the symbolic-regression examples by deciding which user-facing namespaces should be flattened while keeping source folders detailed where useful; include a cleanup pass over operator folders so base types, cross-cutting instrumentation, and candidate-specific implementations are organized consistently without necessarily forcing one folder to equal one namespace
...
- rework the analysis subsystem. Part 3 of [api-ergonomics.md](api-ergonomics.md) made algorithms usable as observation anchors alongside operators, but it converted only `BestMedianWorstAnalysis` and deliberately left the surrounding design alone. The defects found while doing that work, each verified against the code rather than assumed:
  - `AlleleFrequencyAnalyzer` is a stub shipped as public API. It derives from `Analyzer<PopulationSimilarityAnalyzerState>` rather than from its own `AlleleFrequencyAnalyzerState`, which is declared but never used and holds an unread `History` field, and its `AfterInterception` orders the population and then returns without recording anything. Five build warnings point at it
  - anchors are retained in `params IInterceptor<...>[]` positional record parameters in `AllPopulationsAnalysis`, `AlleleFrequencyAnalyzer` and `PopulationSimilarityAnalyzer`, which gives those records reference equality over their anchors; `AllObjectiveVectorsAnalysis` and `BestMedianWorstPerEvaluationAnalysis` retain `ImmutableArray` in a configuration record, which [developer guidelines § 5.4](../docs/contributing/developer-guidelines.md#-54-choose-collection-types-by-ownership-and-meaning) assigns to execution instances. `BestMedianWorstAnalysis` was converted to `ValueArray` and is the shape the rest should follow
  - only `BestMedianWorstAnalysis` accepts algorithm anchors. Converting the remaining five is mechanical, and is the reason to unify the anchor concept once rather than add a second parallel collection to each analysis
  - `ObservationPlan` decides merge conflicts by static type rather than by anchor identity. Observing one interceptor from two analyzers typed at different `TProblem` and `TSearchSpace` — both legal, since the operator roles are contravariant in them — throws `InvalidOperationException: Observation conflict`. Reproduced against the real library
  - `ObservationPlan.Observe` takes three type parameters of which only one is inferable, which is why `ObservationPlanExtensions` exists as a twenty-overload facade. Every new anchor kind costs a hand-written overload pair, as adding algorithm anchors did

  Sequencing matters here. This should run after Part 2 of the ergonomics plan and before its Part 1 namespace work, which splits `Analysis/` by audience and moves `Analyzer` and `Analyzer.Factory` to the root namespace; doing the rework afterwards means moving every one of these files twice. It also needs the analyzer run state disposal question settled, since `AlgorithmRun` carries a `//TODO` asking whether a disposable analyzer run state is the right contract at all — that overlaps the run lifecycle item above. The desired-state analysis API item under [Partly addressed, but still open](#partly-addressed-but-still-open) is the API story for the same subsystem and should be answered by the same design pass
...
- decide where problem-specific operators belong in the source layout. Operators currently fall into three tiers with genuinely different reuse, and all three sit under `Operators/<Role>/` today: general role machinery usable with any candidate and problem (`PipelineRefiner`, `ImprovementCheckingRefiner`, the authoring bases and instrumentation pairs); candidate-specific operators bound to one genotype but any problem over it (`Operators/Mutators/SymbolicExpressionMutators`); and problem-specific operators bound to one problem type (`NumericParameterFittingRefiner`, which requires `SymbolicRegressionProblem` and is the case that raised this). Browsing `Operators/Refiners` therefore mixes universal infrastructure with a leaf only symbolic-regression users can instantiate, and the same will recur for every problem-specific operator. Options include keeping role-first grouping everywhere and accepting domain leaves in general namespaces, colocating problem-specific operators with their problem such as `Problems/DataAnalysis/Regression/Operators`, or separating the tiers by folder while keeping role-first namespaces. Note that colocation would contradict the recorded ownership decision that the operator role remains the primary namespace grouping, so that decision has to be revisited rather than quietly broken. This is the placement taxonomy behind the broader layout item above about organizing operator folders consistently.

  The three tiers were since verified against the code rather than assumed, and the current naming already encodes them. `Operators/Creators/SymbolicExpressionCreators` and `Operators/Mutators/SymbolicExpressionMutators` import only `Genotypes.SymbolicExpressions`, `SearchSpaces.SymbolicExpressions` and `Random`, so they work for any problem over that genotype; `Operators/Refiners/SymbolicRegressionRefiners` imports `DataAnalysis.Regression` and `Problems.DataAnalysis.Regression` and cannot. The genotype-versus-domain naming split is therefore correct today and worth preserving through whatever placement is chosen, and it is recorded in [developer guidelines § 9.3](../docs/contributing/developer-guidelines.md#-93-name-types-for-what-binds-them). What remains open is only where the tiers should live.

  A second instance outside `Operators/` now exists and should be decided together with it: `SymbolicRegressor` and `NumericParameterFitter` were the only two files in `DataAnalysis/Regression/` that imported `Genotypes.SymbolicExpressions`, and they moved to `DataAnalysis/Regression/Symbolic/` so the surrounding regression code — `IRegressor`, the metrics, `LinearScaling`, `BoundedRegressor` — is genotype-agnostic in fact and not merely in intent. That move is folder-only, so the namespace-level dependency remains; promoting the folder to its own namespace is what would enforce it, at the cost of one `using` for anyone touching `SymbolicRegressor`
