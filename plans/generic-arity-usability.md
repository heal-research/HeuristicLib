...
- **The zero-argument construction path is nearly unavailable.** `GeneticAlgorithm.For(problem, …)` constrains six interfaces — `IProblemDefaultCreator`, `IProblemDefaultCrossover` and `IProblemDefaultMutator` on the problem, and the three `IEncodingDefault*` counterparts on the search space. `TravelingSalesmanProblem` is the only problem in the library declaring its three and `PermutationSearchSpace` the only space declaring its three, so permutations are the only encoding where the path exists at all. Every other problem, including every problem a user writes, falls back to `Create(...)` with operators supplied or to direct construction. The diagnostic improved with the arity reduction — a `CS0411` listing all thirteen factory parameters and naming no cause became a `CS0311` naming the missing declaration — but the barrier itself is unchanged. See step 1 below for the half of it that costs nothing.
...
Status: **the arity reduction is done.** All twelve operator roles, the algorithms, the experiments, the factories, the analyzers and the documentation have landed; see [configuration-arity-migration.md](configuration-arity-migration.md), whose exit criteria are all met. Step 3 below *is* that migration and step 7 is done twice — the invariant vocabulary was designed, then renamed once its first users showed which names did not read. Step 2 is settled by decision rather than by implementation, recorded below. Steps 4 and 6 closed with the migration, and step 5 was superseded by it.

**Step 1 is the only one still open**, and only half of it needs evidence. `RoleContractArityTests` guards the achieved shape, so a role reintroducing a configuration type argument fails a test rather than only widening a user's declarations.

1. **Open: have the built-in problems and search spaces declare their defaults.** Makes `For(problem, …)` real rather than a one-encoding special case and removes the worst first-contact failure. The constraint has two independent halves, and only one of them is a design question.

   The **problem half is not a defaults question at all.** Every member of `IProblemDefaultCreator`, `IProblemDefaultCrossover` and `IProblemDefaultMutator` is a `static virtual` returning `null`, which delegates the role to the encoding. So a problem with no opinion still has to declare three interfaces, at full triple arity, in order to add nothing — and until it does, the factory is unavailable to it.

   The constraint exists because the members are **static**: a static interface member can only be invoked through a type parameter constrained to that interface, so there is no way to ask whether a problem happens to declare one. Two ways out were considered.

   **Rejected: having `Problem<TSelf, TCandidate, TSearchSpace>` implement the three interfaces.** It would satisfy the constraint for every problem at once, but a base class naming the three built-in default interfaces is an enumeration of the built-in roles. A consumer adding an operator role in another assembly would still declare its defaults interface per problem, so built-in roles would be privileged over user roles permanently. That contradicts the rule the invariant system settled — nothing enumerates operator roles, and a consumer's role participates on the same terms as a built-in one.

   **Open: make the problem-side defaults instance members at candidate arity.** `ICreator<TCandidate>? CreateDefaultCreator()` on `IProblemDefaultCreator<TCandidate>`, consulted as `(problem as IProblemDefaultCreator<TCandidate>)?.CreateDefaultCreator()`. The constraint then disappears rather than moving: the factory is available to every problem with no declaration, the interface drops `TSelf` and `TSearchSpace`, and no base class is involved, so a consumer's role is reached by the consumer's own factory on identical terms. It also deletes the `problem as TProblem` recovery in `For`, whose own comment concedes the self-type constraint makes it "correct only by convention". The enumeration that remains is an algorithm naming its own slots, which is not the same thing.

   **Why `static` was chosen, recovered from the original design.** It was record value equality: a static member is not a field, so it can never enter a record's generated `Equals` or `GetHashCode`, "enforced by the language rather than by a convention an implementer has to remember". The prototype it was chosen over used instance **auto-properties**, which do produce backing fields and did enter record equality — it passed the suite only because every default operator happened to be a parameterless record. An instance **method** was never the alternative considered, and it has no backing field either, so the reason that decided `static` does not reach it: an interface method member cannot be implemented by an auto-property, so the language enforces the same outcome. What `static` still buys is protection against an implementer deliberately returning a cached field, which the separate rule that every call must return a new operator already forbids for its own reason.

   Two documented costs of `static` are downstream of it and would go. The CRTP self type exists only because reaching a `static abstract` member through an instance is `CS0176`, which forces the concrete type into a type parameter. And the members are `static virtual` returning `null` rather than `static abstract` because "a C# constraint cannot say *call this if the type has it*", so an algorithm must name a fixed role set and a problem must declare every role that algorithm asks about whether or not it has an opinion — the exact limitation instance dispatch does not have. Both shapes are workarounds for `static`, not requirements of the feature.

   Still undecided: whether moving the defaults onto the problem's public instance surface is acceptable, and whether the search space side should follow, where the members are `static abstract` with a non-nullable return and the interface also serves as a factory parameter type for inference.

   The **search space half needs evidence per encoding**, independently of the shape question above., because a default creator, crossover or mutator is public API and changing it changes results for everyone who took it. That is the existing backlog item on choosing defaults per encoding and problem, and it does not block the problem half. Doing the problem half first also improves the remaining failure: the constraint that fails then names only the search space, one axis with one honest reason, instead of blaming a problem for interfaces it has no content for.
2. **Settled: validation is the declared-contract walk, not a resolution attempt.** `SearchConfigurationValidation` walks the configuration graph and aggregates every incompatibility from what operators and search spaces declare. Creating a run performs that walk by default, with `validate: false` to opt out.

   The original plan here was to extend the pass to *also* resolve the graph, so validation and execution would share one rule. That is rejected. **Validation by attempting resolution is not reliable enough to be worth its cost:** resolution can succeed and the algorithm still fail later, because a meta-algorithm resolves its inner algorithms lazily and internally, so a successful trial resolve says nothing about what the run will actually build. A check that passes and then fails anyway is worse than no check, because it is read as an assurance.

   The declared-contract walk has the opposite property. It reasons about what operators state rather than about what one speculative construction happens to do, so a clean report means something stable.

   **Done.** The resolution attempt is gone from `algorithm.Validate(problem)` and `ValidateAndThrow`, and what it used to catch is now caught better. `IExecutionInstanceResolvable.Fits(ExecutionSignature)` decides from type arguments what the authoring bases decide when they bridge, so validation and execution share one rule literally rather than by resemblance; the bridges call it too, which also removed seventeen copies of the mismatch message. A composition that resolves its children under the run it was given forwards to them, and one that adapts its children does not — not forwarding is the default, so an adapting composition is a boundary without writing any code. The capability it used to provide is kept and strengthened: an operator written for one concrete problem used with another is still reported before the run starts, and now so is one inside a cycling algorithm's lazily resolved stage, which no trial resolve would ever have reached.
3. **Done — this is the migration.** Step 1 of the ladder, removing `TProblem` from the configuration layer while execution instances keep it. Prototype the generic resolution mechanism on one role, most likely mutators. Measure the diff the way the evaluator return-type change was measured in the backlog: how much of it is mechanical, how much reaches behavior. Do not proceed to step 2 before step 1 has been judged on that measurement.
...
4. **Done.** The examples and specs no longer show full-arity direct construction. What remains under this heading is the analyzer on the backlog that rewrites explicit construction into the inferring factory, which is tracked there rather than here. The one straggler found after the migration was a `ChooseOneMutator` snippet in [symbolic-regression.md](../docs/guide/domains/symbolic-regression.md); nothing builds the guide snippets, which is the separate backlog gap that let it survive.
5. **Superseded, not pending.** Closed generic aliases were the practitioner-level mitigation for naming `GeneticAlgorithm<RealVector, BoundedRealVectorSearchSpace, TestFunctionProblem>` repeatedly. `GeneticAlgorithm<RealVector>` does not need an alias, so documenting the workaround now would document a workaround for the problem this plan removed. The mechanism still exists and still works for anyone who wants it; it is no longer a mitigation this library has to teach.
6. **Done — step 2 was taken.** The sequencing instruction here ("only after 1–4") outlived its own execution: steps 1 and 2 of the ladder landed together as the migration, for the reason given under the ladder table. What the two search spaces over `BoolVector` were kept for is now served by the invariant system, which reproduces the six-cell table from declarations. Step 3 remains rejected.
7. **Done.** The invariant vocabulary was designed and then revised once real users of it existed: `ICandidateInvariant<TCandidate>` with `Holds` and `Implies`, and `IOperatorContract<TCandidate>` with `Ensures` and `Requires`. `Ensures` survived the revision because a creator *establishes* an invariant where a mutator *preserves* one, and only "ensures this of my output" covers both. The rules behind the naming are §§ 8.12–8.13 of the developer guidelines, and the system is documented in [invariants-and-validation.md](../docs/guide/fundamentals/invariants-and-validation.md).
