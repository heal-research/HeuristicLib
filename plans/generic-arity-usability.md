...
Generic type arguments remain HeuristicLib's compatibility mechanism at execution boundaries. Public algorithm configurations and operator roles now name only `TCandidate`. Resolution methods introduce `TSearchSpace`, `TProblem` and, for stateful roles, `TSearchState` when an execution instance is created. It remains a compile-time error to hand a permutation crossover to a real vector algorithm.

Before this migration, a configured genetic algorithm named its candidate, search space and problem. Stateful roles and composed algorithms added more arguments. The current API reduces a standard algorithm to `GeneticAlgorithm<TCandidate>` and introduces the remaining types only where execution needs them.
...
- **The zero-argument construction path now reports missing recommendations at construction.** `GeneticAlgorithm.For(problem, …)` used to constrain six interfaces and was available only for permutations. It now accepts every problem, resolves supplied values before problem and search space recommendations, then throws one `InvalidOperationException` naming every unresolved required parameter. See step 1 below.
...
> Split a search space when **membership** or the **data operators read** differ. Use problem recommendations when only **suitability** differs.

Under that rule the QAP accepting `EdgeRecombinationCrossover` from the search space recommendations is an argument for `QuadraticAssignmentProblem` declaring its own recommendation, not for a new search space.
...
**Every ergonomics step is now closed.** `RoleContractArityTests` guards the achieved shape, so a role reintroducing a configuration type argument fails a test rather than only widening a user's declarations.

1. **Done: replace operator defaults with recommendations.** The old `IProblemDefault*` and `IEncodingDefault*` contracts are gone. The source neutral `IRecommends<TOperator>` contract exposes `TryCreateRecommendedOperator`, which returns `true` with a fresh operator or `false` with `null` when the source declines in its current state. A problem or search space implements the closed generic interface for each recommendation it can make. No role specific recommendation interfaces exist.

   The instance methods add no stored fields, so they do not participate in record equality. They can inspect the current problem or search space without receiving `this` as a separate parameter. Every call still returns a fresh operator because operator reference identity anchors observations.

   `For(problem, …)` retains `Problem<TProblem, TCandidate, TSearchSpace>` as its inference anchor. `For(searchSpace, …)` now takes `ISearchSpace<TCandidate>`, which infers the candidate without the old marker interface. Neither overload constrains recommendation availability.

   Each factory takes a supplied value first, then tries a problem recommendation and then a search space recommendation. A source that returns `false` lets the factory continue to the next source. Public `OperatorRecommendationResolution` accepts the non generic `IProblem` and `ISearchSpace` contracts, then works generically over the requested operator contract. The non generic problem contract exposes objective directions because they do not depend on candidate or search space types. Only the algorithm lists the parameters it requires, so another algorithm can request a consumer defined operator role without changing shared machinery. Optional roles are not populated from recommendations. The factory collects every unresolved required parameter and throws one `InvalidOperationException` during construction. This loses the old compile-time refusal deliberately: its constraints expressed a conjunction across both sources while the real rule is a disjunction per parameter.

   `TravelingSalesmanProblem` now declares only a crossover recommendation. `PermutationSearchSpace` recommends the creator, crossover and mutator. The remaining search spaces still need evidence before gaining recommendations, which stays on the developer backlog together with provenance metadata for reproducibility.
